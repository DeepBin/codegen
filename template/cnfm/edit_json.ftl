...
<body style="overflow-x:hidden;" ng-controller="ctrl" ht-load="get${class}Json?id=${r"${"}param.id}" ng-model="data">
	<form id="form-param-edit" action="#" name="form" method="post">
		<div>
			<%@ include file="/WEB-INF/view/${system}/${package}/${classVar}CommonView.jsp"%>
			<div class="row mt20">
				<div class="col-md-12 text-center mt20 buttons">
					<button type="button" class="btn btn-default btn-blue btn-lg plr-80 mr30" ng-click="closeLayer()">
                    	关闭
                	</button>
					<button type="button" class="btn btn-info border-ra0 btn-lg plr-80" ng-click="saveForm('save${class}')" ng-disabled="form.$invalid">
                    	提交
                	</button>
				</div>
			</div>
